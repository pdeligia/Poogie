...
procedure {:inline 1} $create_machine(m: Machine) returns (r: int);
  modifies $CurrMid, $MType, $Heap, $State, $IsHalted, $Raised, $InboxSize;

implementation {:inline 1} $create_machine(m: Machine) returns (mid: int)
{
  $bb0:
    assume $CurrMid > 0;
    mid := $CurrMid;
